## HTTP Server
### Цель:
    Разработать простой HTTP сервер на языке GO.
    Конечная точка должна рассчитывать возраст на основе даты рождения.
### Задачи:
...
ВАЖНО! В данном примере формула расчета возраста крайне упрощена, и моменты, касающиеся високосного года и различного количества дней в месяцах, отброшены в сторону. Поэтому ошибка в расчетах может составлять несколько дней. Параметры для расчета: год — 365 дней, месяц — 31. Так же не склоняются слова "лет" "месяцев", "дней".
